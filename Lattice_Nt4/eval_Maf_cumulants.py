    ...
    with open(file_name, 'r', encoding='utf-8') as file:
        for line in file:
            if '=' in line:
                ...
                if name.startswith('beta_range_'):
                    mass_key = float(name[len('beta_range_'):])
                    beta_min_val, beta_max_val, beta_range_val = map(float, value.split(','))
                    beta_ranges[mass_key] = (beta_min_val, beta_max_val, beta_range_val)
                elif name in ['volumes', 'masses']:
                    params[name] = [float(v.strip()) for v in value.split(',')]
                else:
                    try:
                        params[name] = float(value)
                    except ValueError:
                        params[name] = value
    return params, beta_ranges


#Reading parameters from input file, like the file to read and minmax conditional parameters
input_params, beta_ranges = read_input_file('input_eval.txt')
...
model_weights_filename = input_params.get('model_weights', f'path/to/model_weights_bs{bs}_nummade{num_made}_loop{loop_num}')

full_model_path = os.path.join(base_modeldir, model_weights_filename)

outputdirectoryname = os.path.join(input_params.get('outputdirname', f'OutputDirName_{bs}_made{num_made}'), f'run{loop_num}')
...
for mass in masses:
  ...
  if mass not in beta_ranges:
      raise ValueError(f"No beta range defined for mass {mass}")
  beta_min_mass, beta_max_mass, beta_range_mass = beta_ranges[mass]
  betas = np.linspace(beta_min_mass, beta_max_mass, int(beta_range_mass))
  ...
  for vol in volumes:
    for beta in betas:
      ...
      mean = chi1
      sus = chi2 - chi1*chi1
      B3 = (chi3 - 3*chi2*chi1 + 2*(chi1**3))/sus**(3/2)
      B4 = (chi4 - 4*chi3*chi1 + 6*chi2*chi1*chi1 - 3*chi1*chi1*chi1*chi1)/sus/sus
      tmp = np.array([vol, mass, beta, mean, sus, B3, B4, chi_std])
      
      if result.shape[0]>0:
        result = np.concatenate([result, np.reshape(tmp,(1,8))], axis=0)
      else:
        result = np.reshape(tmp,(1,8))
