    ...
    with open(file_name, 'r', encoding='utf-8') as file:
        for line in file:
            if '=' in line:
                ...
                # Treat certain parameters as lists
                if name in ['volumes', 'masses', 'betas', 'as', 'bs', 'cs']:
...
input_params = read_input_file('input_eval.txt')

num_made = int(input_params.get('num_made', 8))
run_num = 1  # default if not provided

# Override from command-line arguments
# Usage: python eval_MAF.py <num_made> <run_num>
if len(sys.argv) > 1:
...
model_weights_dir = input_params.get('model_weightsdir', 'trained_models/default')
output_dirname = input_params.get('outputdirname', 'eval_results_default')

# Construct full paths if needed
model_weights_filename = os.path.join(model_weights_dir, f'GMM_made{num_made}_run{run_num}')
#outputdirectoryname = os.path.join(output_dirname, f'made{num_made}_run{run_num}')
outputdirectoryname = os.path.join(output_dirname, f'made{num_made}', f'run{run_num}')
...
volumes = input_params['as']
masses = input_params['bs']
betas = input_params['cs']
    ...
    for vol in volumes:
        for beta in betas:
            outputfilename = os.path.join(
                outputdirectoryname,
                f'GMM_vol{round(vol, 2)}_mass{round(mass, 2)}_beta{round(beta, 2)}.npz'
            )
            os.makedirs(os.path.dirname(outputfilename), exist_ok=True)

            cond0 = np.concatenate([
                norm_vol(vol)*np.ones((n_samples, 1)),
                norm_mass(mass)*np.ones((n_samples, 1)),
                norm_beta(beta)*np.ones((n_samples, 1))], axis=1)

            current_kwargs0 = {f"maf_{i}": {'conditional_input': cond0} for i in range(num_made)}
            ...
            result = (vol, mass, beta, chis, acts)
